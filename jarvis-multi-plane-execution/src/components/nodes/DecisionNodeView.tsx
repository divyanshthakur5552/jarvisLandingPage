...
export const DecisionNodeView: React.FC<DecisionNodeViewProps> = ({
  ...
  return (
    ...
    >
      <div
        className="relative w-full h-full rounded-[14px] bg-[#141419] border transition-all duration-300 p-3 flex flex-col justify-center"
        style={{
          borderColor,
          boxShadow,
        }}
      >
        ...
        <div className="absolute -right-3 top-1/2 -translate-y-1/2 w-3 h-3 rounded-full bg-[#1e1e24] border border-[#3a3a45] flex items-center justify-center z-10">
          <div className={`w-1.5 h-1.5 rounded-full ${isRunning ? 'bg-[#22c55e]' : isDone ? 'bg-[#10b981]' : 'bg-[#3a3a45]'}`} />
        </div>

        {/* Active Running Chip */}
        {isRunning && (
          <div className="absolute -top-3 right-3 px-2 py-0.5 rounded-full bg-[#111116] border border-[#22c55e] flex items-center gap-1.5 shadow-lg animate-fade-in z-20">
            <span className="w-1.5 h-1.5 rounded-full bg-[#22c55e] animate-ping" />
            <span className="text-[10px] font-mono font-medium tracking-tight text-[#4ade80]">evaluating</span>
          </div>
        )}
        ...
        {isDone && (
          <div className="absolute -top-2.5 right-3 px-1.5 py-0.5 rounded-full bg-[#111116] border border-[#10b981]/50 flex items-center gap-1 shadow-md z-20">
            ...
            <span className="text-[9px] font-mono text-[#10b981]">decided</span>
          </div>
        )}
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-shrink-0">
            ...
            >
              <NodeIcon name={node.iconName} size={16} className="text-zinc-950 stroke-[2.2]" />
            </div>
          </div>

          <div className="flex flex-col min-w-0 pr-1">
            <h3 className="text-[14px] font-semibold text-[#f4f4f5] tracking-tight leading-snug line-clamp-2">
              {node.title}
            </h3>
          </div>
        </div>
